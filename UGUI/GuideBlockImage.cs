...
namespace PK2.UI
{
    ...
    /// </summary>
    public class GuideBlockImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
    {
        /// <summary>
        ...
        [UnityEngine.Tooltip("Mask的总缩放")]
        [SerializeField] float MaskScale = 1.3f;
        private float _oldMaskScale = 1.3f;
        [UnityEngine.Tooltip("根据UI宽高比调整Mask的大小")]
        [SerializeField] private AnimationCurve WidthScaleCurve;
        [SerializeField] private AnimationCurve HeightScaleCurve;

        private void Update()
        {
            if(blockRectTransform!=null && blockRectTransform.hasChanged)
                SetAllDirty();
            if (Math.Abs(MaskScale - _oldMaskScale) > 0.001)
            {
                _oldMaskScale = MaskScale;
                SetAllDirty();
            }
        }

        public void SetBlockRectTransform(RectTransform rectTransform)
        {
            blockRectTransform = rectTransform;
            SetAllDirty();
        }
        ...
        public override Material material
        {
            get
            {
                if (m_Material != null)
                    return m_Material;
#if UNITY_EDITOR
                if (UnityEngine.Application.isPlaying && activeSprite && activeSprite.associatedAlphaSplitTexture != null)
        ...
        private void GenerateSlicedSprite(VertexHelper toFill)
        {
            ...
            {
                //var blockPos = RectTransformUtility.WorldToScreenPoint(this.canvas.worldCamera, blockRectTransform.position);
                //var thisPos = RectTransformUtility.WorldToScreenPoint(this.canvas.worldCamera, rectTransform.position);
                //Vector3[] corners = new Vector3[4];
                //blockRectTransform.GetWorldCorners(corners);

//                var topLeft = RectTransformUtility.WorldToScreenPoint(this.canvas.worldCamera, corners[0]);
//                var bottomRight = RectTransformUtility.WorldToScreenPoint(this.canvas.worldCamera, corners[2]);
                float blockWidth = blockRectTransform.rect.width ;
                float blockHeight = blockRectTransform.rect.height;
                var blockPos = blockRectTransform.anchoredPosition;
                
                blockPos = blockPos + (-(blockRectTransform.pivot - new Vector2(0.5f, 0.5f))) * new Vector2(blockWidth, blockHeight);
                
                float ratio = blockWidth / blockHeight;

                float widthScale = WidthScaleCurve.Evaluate(ratio);
                float heightScale = HeightScaleCurve.Evaluate(ratio);
                
                Vector2 offset = blockPos;
                
                {
                    ...
                    s_VertScratch[1].x += -(blockWidth - sprite.border.x - sprite.border.z) / 2;
                    s_VertScratch[1].y += -(blockHeight - sprite.border.y - sprite.border.w) / 2;
                    s_VertScratch[1] *= new Vector2(widthScale, heightScale);
                    s_VertScratch[1] *= MaskScale;
                    s_VertScratch[1] += offset;
                
                    s_VertScratch[2] = Vector2.zero;
                    s_VertScratch[2].x += (blockWidth - sprite.border.x - sprite.border.z) / 2;
                    s_VertScratch[2].y += (blockHeight - sprite.border.y - sprite.border.w) / 2;
                    s_VertScratch[2] *= new Vector2(widthScale, heightScale);
                    s_VertScratch[2] *= MaskScale;
                    s_VertScratch[2] += offset;
                }
            }
            else
            {
